#!/usr/bin/env python3
"""
Add PayTouch2 as a payout option for ADMIN personal payouts
This allows PayTouch2 to appear in the admin personal payout dropdown
"""

from database import get_db_connection

def add_paytouch2_admin_routing():
    """Add PayTouch2 routing for admin personal payouts"""
    ...
    try:
        with conn.cursor() as cursor:
            # Check if PayTouch2 admin routing already exists
            cursor.execute("""
            ...
            if existing:
                if existing['is_active']:
                    print("✅ PayTouch2 admin routing already exists and is active")
                    ...
                    """, (existing['id'],))
                    conn.commit()
                    print("✅ PayTouch2 admin routing activated")
                    print(f"   Route ID: {existing['id']}")
            else:
                # Insert new routing
                cursor.execute("""
                ...
                """)
                conn.commit()
                print("✅ PayTouch2 admin routing created successfully")
...
if __name__ == '__main__':
    print("🔧 Adding PayTouch2 to Admin Personal Payout Options\n")
    success = add_paytouch2_admin_routing()
    ...
    if success:
        ...
        print("   4. PayTouch2 should now appear in the Payment Gateway dropdown")
    else:
