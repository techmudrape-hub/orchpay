...
def get_connection_pool():
    ...
    OPTIMIZED FOR 3 INSTANCES BEHIND LOAD BALANCER:
    - Each instance: 4 workers × 1000 connections = 4000 potential connections
    - 3 instances × 15 max connections = 45 total connections to RDS
    - Safe for most RDS instances (default max_connections = 150+)
    """
    global _pool
    if _pool is None:
        print("Initializing OPTIMIZED database connection pool for load balancer...")
        _pool = PooledDB(
            creator=pymysql,
            maxconnections=15,        # REDUCED: Max 15 connections per instance (3 instances = 45 total)
            mincached=3,              # REDUCED: Keep 3 idle connections ready
            maxcached=8,              # REDUCED: Max 8 idle connections
            maxshared=0,              # No shared connections (thread-safe)
            blocking=True,            # Block if pool exhausted (don't fail)
            maxusage=500,             # REDUCED: Recycle connection after 500 uses (more frequent recycling)
            setsession=[],
            ping=1,                   # Check connection health (0=never, 1=default, 2=when fetched, 4=when returned, 7=always)
            host=Config.DB_HOST,
            user=Config.DB_USER,
        ...
        print(f"✓ Connection pool initialized for LOAD BALANCER:")
        print(f"  - Max connections per instance: 15")
        print(f"  - Total across 3 instances: 45")
        print(f"  - Min cached: 3, Max cached: 8")
        print(f"  - Connection recycling: every 500 uses")
    return _pool
...
def close_connection(connection):
    """
    Properly close/return connection to pool
    Call this in finally blocks to ensure connections are returned
    """
    if connection:
        try:
