"""
AU Bank Payin Service
Generates dynamic UPI intent links for AU Bank QR code
"""
...
class AuBankService:
    def __init__(self):
        self.payee_vpa = "aimpay@aubank"
        self.payee_name = "Sumit Kumar Tripathi"
        self.orgid = "159765"
        self.mc = "5511"
        self.mode = "16"
        self.purpose = "00"
        self.sign = "MEQCIACsN5DjH16La8438Af2mE+gbKp++WbHZsqJUOFeLH+7AiBLGhaVclYjrQerW01kMUFghxdJcEX60B7pFoSDYO0rPQ=="
    ...
    def create_payin_order(self, merchant_id, order_data):
        ...
        try:
            ...
            with conn.cursor() as cursor:
                ...
                merchant_order_id = order_data.get('orderid') or f"AUBANK_{merchant_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                
                # Create internal transaction ID
                txn_id = f"AUBANK_{merchant_id}_{merchant_order_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}"
                ...
                if not callback_url:
                    ...
                    callback_url = f"{base_url}/api/callback/aubank/payin"
                ...
                """, (
                    ...
                    'INITIATED', 'AU_BANK', merchant_order_id,
                    callback_url
                ))
