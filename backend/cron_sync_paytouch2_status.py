"""
Cron Job: Sync PayTouch2 Payout Status
Automatically check and update status for pending PayTouch2 transactions
...
def sync_paytouch2_status():
    """Check and update status for pending PayTouch2 transactions"""
    
    print("=" * 80)
    print(f"PayTouch2 Status Sync - {datetime.now()}")
    print("=" * 80)
    ...
    try:
        with conn.cursor() as cursor:
            # Find PayTouch2 transactions that are pending and older than 5 minutes
            # (give PayTouch2 time to send callback first)
            cursor.execute("""
                SELECT 
            ...
            if not pending_txns:
                print("No pending PayTouch2 transactions found")
                return
            
            print(f"\nFound {len(pending_txns)} pending PayTouch2 transactions")
            print("-" * 80)
            ...
            for txn in pending_txns:
                ...
                try:
                    ...
                    paytouch2_status = status_result.get('status')
                    paytouch2_utr = status_result.get('utr')
                    
                    # Map PayTouch2 status
                    status_map = {
                        'SUCCESS': 'SUCCESS',
                        'PENDING': 'QUEUED',
                        'FAILED': 'FAILED',
                    ...
                    mapped_status = status_map.get(paytouch2_status, 'QUEUED')
                    
                    print(f"  PayTouch2 Status: {paytouch2_status} → {mapped_status}")
                    ...
                    if mapped_status == 'SUCCESS':
                        ...
                        cursor.execute("""
                            ...
                            WHERE txn_id = %s
                        """, (mapped_status, paytouch2_utr, txn['txn_id']))
                        ...
                        cursor.execute("""
                            ...
                            SET status = %s, 
                                ...
                                updated_at = NOW()
                            WHERE txn_id = %s
                        """, (mapped_status, 'Failed at PayTouch2', txn['txn_id']))
