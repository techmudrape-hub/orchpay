"""
Airpay Payin API Routes
Handles merchant payin operations through Airpay
"""
...
airpay_bp = Blueprint('airpay', __name__, url_prefix='/api/airpay')

@airpay_bp.route('/order/create', methods=['POST'])
@jwt_required()
def create_airpay_order():
    """
    Create Airpay payin order (for merchant dashboard)
    ...
    try:
        ...
        try:
            with conn.cursor() as cursor:
                ...
                response_data = {
                    ...
                    'airpay_txn_id': result['airpay_txn_id']
                }
        ...
        print(f"Create Airpay order error: {e}")
        return jsonify({'success': False, 'message': 'Internal server error'}), 500

@airpay_bp.route('/status/<txn_id>', methods=['GET'])
@jwt_required()
def check_airpay_status(txn_id):
    """Check Airpay payment status by transaction ID"""
    try:
        current_merchant = get_jwt_identity()
        ...
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE txn_id = %s AND merchant_id = %s AND pg_partner = 'Airpay'
                ...
                status_result = airpay_service.check_payment_status(order_id)
                ...
                if airpay_status == 'SUCCESS' and txn['status'] != 'SUCCESS':
                    ...
                    wallet_result = wallet_svc.credit_unsettled_wallet(
                        ...
                        amount=net_amount,
                        description=f"PayIn received (Airpay) - {order_id}",
                        reference_id=txn_id
                    )
                    ...
                    admin_wallet_result = wallet_svc.credit_admin_unsettled_wallet(
                        ...
                        description=f"PayIn charge (Airpay) - {order_id}",
                        reference_id=txn_id
                    )
                ...
                elif airpay_status in ['FAILED', 'EXPIRED'] and txn['status'] not in ['FAILED', 'CANCELLED']:
                    # Update to FAILED
                    cursor.execute("""
                        UPDATE payin_transactions
                        SET status = 'FAILED',
        ...
        print(f"Check Airpay status error: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({'success': False, 'message': str(e)}), 500

@airpay_bp.route('/status/order/<order_id>', methods=['GET'])
@jwt_required()
def check_airpay_status_by_order(order_id):
    """Check Airpay payment status by order ID"""
    try:
        current_merchant = get_jwt_identity()
        ...
        try:
            with conn.cursor() as cursor:
                ...
                cursor.execute("""
                    SELECT * FROM payin_transactions
                    WHERE order_id = %s AND merchant_id = %s AND pg_partner = 'Airpay'
                ...
                if txn['status'] in ['SUCCESS', 'FAILED', 'CANCELLED']:
                    # If SUCCESS but bank_ref_no is NULL, fetch from Airpay
                    if txn['status'] == 'SUCCESS' and not txn.get('bank_ref_no'):
                        print(f"Transaction is SUCCESS but bank_ref_no is NULL, fetching from Airpay...")
                ...
                print(f"Checking status with order_id: {order_id}")
                
                # Check status from Airpay using the order_id
                status_result = airpay_service.check_payment_status(order_id)
                ...
                if not status_result.get('success'):
                    ...
                    return jsonify({
                        'success': True,
                        'status': txn['status'],
                        'message': 'Could not fetch latest status from Airpay',
                ...
                if airpay_status == 'SUCCESS' and txn['status'] != 'SUCCESS':
                    ...
                    if not wallet_already_credited:
                        ...
                        wallet_result = wallet_svc.credit_unsettled_wallet(
                            ...
                            description=f"PayIn received (Airpay status check) - {order_id}",
                            reference_id=txn_id
                        )
                        ...
                        admin_wallet_result = wallet_svc.credit_admin_unsettled_wallet(
                            ...
                            description=f"PayIn charge (Airpay status check) - {order_id}",
                            reference_id=txn_id
                        )
                ...
                return jsonify({
                    'success': True,
                    'status': airpay_status or txn['status'],
                    'transaction': {
                        'txn_id': txn_id,
                        'order_id': order_id,
                        'amount': float(txn['amount']),
