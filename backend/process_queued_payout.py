#!/usr/bin/env python3
"""
Process Queued Payouts via Mudrape (TODAY ONLY)
This script takes queued payouts and processes them via Mudrape API.
It updates pg_partner to Mudrape and processes the payout.
"""
...
def process_queued_payout(merchant_id, dry_run=True):
    """
    Process queued payouts for a specific merchant via Mudrape (TODAY ONLY).
    ...
    try:
        with conn.cursor() as cursor:
            ...
            print(f"\n{'='*80}")
            print(f"Process Queued Payouts via Mudrape (TODAY ONLY)")
            print(f"{'='*80}")
            ...
            print(f"\n{'='*80}")
            print("Finding TODAY's queued payouts...")
            print(f"{'='*80}\n")
            ...
            # Find queued payouts from TODAY (any pg_partner)
            cursor.execute("""
                SELECT 
                    txn_id,
                    reference_id,
            ...
            for idx, payout in enumerate(queued_payouts, 1):
                ...
                if dry_run:
                    print(f"   🔍 WOULD UPDATE pg_partner to Mudrape")
                    print(f"   🔍 WOULD PROCESS via Mudrape API")
                else:
                    try:
                        # Step 1: Update pg_partner to Mudrape
                        print(f"   📝 Updating pg_partner to Mudrape...")
                        cursor.execute("""
                            UPDATE payout_transactions 
                            SET pg_partner = 'Mudrape', updated_at = NOW()
                            WHERE txn_id = %s
                        """, (txn_id,))
                        conn.commit()
                        
                        # Step 2: Call Mudrape API
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python process_queued_payout.py <merchant_id> [--live]")
        print("\nExamples:")
        print("  python process_queued_payout.py 7619163479          # Dry run")
        print("  python process_queued_payout.py 7619163479 --live   # Process payouts")
        sys.exit(1)
    ...
    success = process_queued_payout(merchant_id, dry_run)
