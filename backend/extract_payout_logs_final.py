#!/usr/bin/env python3
"""
Extract complete payout data with timestamps for specific order IDs
"""
...
def main():
    ...
    try:
        with conn.cursor() as cursor:
            # Query payout_transactions table
            ...
            # Query admin_wallet_transactions
            ...
            for txn in wallet_txns:
                print(f"  Order: {txn.get('order_id', 'N/A')}")
            ...
            # Query merchant_wallet_transactions
            ...
            query = f"""
                ...
                FROM callback_logs
                WHERE order_id IN ({placeholders})
                   OR pg_order_id IN ({placeholders})
                ORDER BY created_at DESC
            """
            ...
            for cb in callbacks:
                print(f"  Order: {cb.get('order_id', 'N/A')}")
                print(f"  PG Order: {cb.get('pg_order_id', 'N/A')}")
                ...
                all_data.append({
                    ...
                    'order_id': cb.get('order_id') or cb.get('pg_order_id'),
                    'data': cb
                })
    ...
    report_file = f'payout_complete_report_{timestamp}.txt'
    json_file = f'payout_complete_data_{timestamp}.json'
