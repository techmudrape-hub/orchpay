...
Usage:
    python mark_merchant_payins_failed.py --merchant <merchant_id> --start "YYYY-MM-DD HH:MM:SS" --end "YYYY-MM-DD HH:MM:SS"
"""
...
def mark_payins_failed_and_callback(merchant_id, start_time, end_time, dry_run=True):
    ...
    try:
        ...
        print(f"Callback URL: {transactions[0]['payin_callback_url']}")
        ...
        for txn in transactions:
            try:
                ...
                if txn['payin_callback_url']:
                    callback_payload = {
                        'order_id': txn['order_id'],
                        'pg_order_id': txn['pg_order_id'],
                    ...
                    try:
                        response = requests.post(
                            txn['payin_callback_url'],
                            json=callback_payload,
                            timeout=10
                        )
                        
                        if response.status_code == 200:
                            print(f"  ✓ Callback sent successfully to {txn['payin_callback_url']}")
