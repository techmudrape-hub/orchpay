"""
ClocksPay Callback Routes
Handles payin callbacks from ClocksPay payment gateway
"""
...
clockspay_callback_bp = Blueprint('clockspay_callback', __name__, url_prefix='/api/callback')

@clockspay_callback_bp.route('/clockspay/payin', methods=['GET'])
def clockspay_payin_callback():
    """
    Callback endpoint for ClocksPay payin status updates
    ClocksPay sends callback via GET method with query parameters
    ...
    - utr: Bank UTR / RRN (may be empty in pending state)
    """
    try:
        # Get callback data from ClocksPay - sent as GET parameters
        ...
        print("=" * 80)
        print("ClocksPay Payin Callback Received")
        print("=" * 80)
        ...
        try:
            with conn.cursor() as cursor:
                ...
                if not txn:
                    ...
                    if recent_txns:
                        print(f"Recent ClocksPay payin transactions:")
                    ...
                    if already_credited:
                        ...
                        try:
                            ...
                            credit_result = wallet_svc.credit_unsettled_wallet(
                                ...
                                description=f"PayIn received (ClocksPay) - {order_id}",
                            ...
                            admin_credit_result = wallet_svc.credit_admin_unsettled_wallet(
                                ...
                                description=f"PayIn charge (ClocksPay) - {order_id}",
                                reference_id=txn['txn_id']
                            )
                ...
                print("=" * 80)
                print("ClocksPay Payin Callback processed successfully")
                print("=" * 80)
                ...
                try:
                    # Get callback URL - ALWAYS use transaction callback_url if present
                    callback_url = None
                    
                    # Step 1: Check transaction callback_url (PRIMARY SOURCE)
                    if txn.get('callback_url'):
                        callback_url = txn['callback_url'].strip()
                        # Skip if it's the internal ClocksPay callback URL
                        if callback_url and 'api.orchpay.in/api/callback/clockspay' in callback_url:
                    ...
                    # Step 2: If no valid callback URL in transaction, check merchant_callbacks table
