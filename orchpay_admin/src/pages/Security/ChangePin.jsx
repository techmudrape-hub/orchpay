...
export default function ChangePin() {
  ...
  const checkPinStatus = async () => {
    ...
    try {
      ...
      // Default to false if error (assume PIN not set)
      setHasPinSet(false)
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true)
    try {
      const response = await adminAPI.changePin(
      ...
      if (response.success) {
        ...
        window.dispatchEvent(new Event('adminPinSet'))
      }
    } catch (error) {
  ...
  const handleDeletePin = async () => {
    setDeletingPin(true)
    try {
      const response = await adminAPI.deletePin()
      if (response.success) {
        ...
        window.dispatchEvent(new Event('adminPinDeleted'))
      }
    } catch (error) {
      ...
      <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete your transaction PIN? You will need to set a new PIN before making any transactions.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
